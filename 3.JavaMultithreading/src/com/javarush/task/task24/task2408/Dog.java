    ...
    public Sayable toSayable(final int i) {
        class DogPet extends SuperDog implements Sayable {
            ...
            public String say() {
                String result = "";
                if (i < 1) {
                    result = String.format("%s спит.", getSuperName());
                } else {
                    result = String.format("%s лает г%sв! %s", getSuperName(), repeat("а", i), getFormattedDate());
                }
                return result;
            }
            ...
            private String getFormattedDate() {
                return transformPidorasiny(formatter.format(new Date()));
            }
        }
        return new DogPet();
    }

    private static String transformPidorasiny(String pidorasina) {
        String result = "";
        try (ByteArrayOutputStream outputStream = new ByteArrayOutputStream()){
            outputStream.write(pidorasina.getBytes());
