...
public class Solution {
    ...
    public static void sort(List<Stock> list) {
        list.sort(Comparator.comparing(Solution::extractName, String::compareTo)
                .thenComparing(Solution::extractDate, Solution::compareDate)
                .thenComparing(Comparator.comparingDouble(Solution::extractChange).reversed()));
    }

    private static String extractName(Stock stock) {
        return (String) stock.get("name");
    }

    private static Date extractDate(Stock stock) {
        return (Date) stock.get("date");
    }
    ...
    private static int compareDate(Date d1, Date d2) {
        ...
        return date1.isAfter(date2) ? -1 : date1.isEqual(date2) ? 0 : 1;
    }

    private static Double extractChange(Stock stock) {
        Double change;
        if (!stock.containsKey("change")) {
            ...
            change = (double) stock.get("change");
        }
        return change;
    }

    private static int compareStocksByChange(Stock stock1, Stock stock2) {
...
//            for (int j = 0; j < 50; j++) {
//                stocks.add(new Stock("Fake Applied Materials, Inc.", "AMAT", Math.random(), randomDate));
//            }
//        }
